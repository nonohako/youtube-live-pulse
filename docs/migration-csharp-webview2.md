...
진행 기록(2026-09-25): C# 공개 감시·영상 조회수·클라우드 폴링·SQLite 백업과 기존 HTML 화면의 상태/분석/설정/가져오기를 연결했다. 원본 Electron JSON은 보존한 채 2채널·245시리즈·661,305표본을 개인용 DB와 백업으로 검증했고, `%LOCALAPPDATA%/Programs/LivePulseNative`의 자체 포함 앱을 숨김 트레이에서 실행 중이다. 네이티브 Fly 인증 읽기 동기화는 여러 차례 성공했으며 1분 간격 관측값이 저장됐다. 실제 기록 복사본의 WebView 화면에서 두 채널 카드를 확인했고 닫은 뒤 브라우저 프로세스가 종료됐다. Windows 자동 시작 항목은 네이티브 실행 파일로 전환했으며 이전 Electron 명령은 보존했다. 실방송 알림/Chrome 실동작과 동등 데이터 장기 메모리 비교는 남아 있다. 공개 운영 릴리스는 계속 Electron v1.12.1이다. 아래 '준비 작업'은 작성 당시 문구이며 현재 상태는 `handoff.md`와 `native/README.md`를 따른다.
