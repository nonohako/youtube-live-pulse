...
진행 기록(2026-09-25): C# 공개 감시·영상 조회수·클라우드 폴링·SQLite 백업과 기존 HTML 화면의 상태/분석/설정/가져오기 연결을 완료했다. 자체 포함 개인용 실행 파일을 만들고 격리 데이터에서 UI 동작과 창 닫기 후 WebView 종료를 확인했다. `native/prepare-personal.ps1`은 Electron을 종료한 뒤 원본 JSON 복사, 해시 확인, SQLite 이전·검증 및 첫 백업을 준비한다. 실제 설치 앱 데이터 전환, 네이티브 Fly 인증 동기화, Windows 알림/Chrome 실동작, 장기 동등 데이터 메모리 관찰은 아직 진행하지 않았다. 운영 앱은 계속 Electron v1.12.1이다. 아래의 '준비 작업' 문구는 문서 작성 당시 상태이며, 현재 실행 상태는 `handoff.md`와 `native/README.md`를 따른다.
...
### 2026-09-25 개인용 범위 조정 — 현재 작업에 우선 적용

사용자는 혼자 쓰는 앱이며 과도한 세부 검증보다 실제 동작하는 앱으로 연결하는 진도를 요청했다. 기존 기능을 유지하되 아래 세 묶음으로 진행한다. 이미 통과한 개별 검증을 반복하거나 기반 계층을 추가로 일반화하지 않는다.

1. **백그라운드 연결:** 현재 감시기·SQLite·클라우드 페이지 동기화를 한 앱에 연결하고 주기 실행/오류 표시/백업을 공유한다.
2. **실제 화면 연결:** 기존 HTML/CSS/JS에 실제 상태, 차트 기록, 설정, 채널 관리, XLSX 가져오기를 연결한다. 부족한 로컬 영상 조회수 수집도 기존 동작에 맞춰 마무리한다.
3. **개인용 실행 확인:** 알림·Chrome 열기·시작 옵션·중복 실행 방지를 연결하고, 격리된 실제 기록으로 감시/동기화/차트/닫기·재열기를 한 번에 확인한다. 전환 전 백업과 수동 복귀 방법을 남긴다.

공개 배포용 자동 업데이트, 구 설치본 자동 업그레이드, 여러 설치 환경 검증, 범용 역변환 도구는 공개 배포 요청 시 진행한다. 개인용 실행본의 완료 조건에 넣지 않는다. 원본 기록 보존, 토큰 비노출, 방송 중복 열기 방지, 창을 닫으면 WebView 해제는 유지한다. 아래 기존 6~7단계의 배포 절차는 공개 릴리스에만 적용한다.
