...
완성된 설치 파일은 `dist/LivePulse-Setup-1.4.0.exe`에 생성됩니다. 설치 후 앱 설정에서 “Windows 로그인 때 자동 실행”이 켜져 있으면 다음 로그인부터 숨김 상태로 트레이에서 시작합니다.
...
- 성장 분석은 완료된 날짜별 마지막 측정값을 기준으로 하며, 측정일이 비면 증가분을 경과 일수로 나눕니다. 오늘 값은 원본 차트에만 표시하고 성장 분석과 선택 구간 계산에서는 제외합니다.
