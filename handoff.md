...
## Midpoint audit and next execution scope

The user clarified this is a personal single-user app and requested GPT-6 Sol high without excessive incremental validation. The monitor, cloud loop, WebView state, channel/settings/refresh/XLSX/connection actions, video statistics and personal executable are now connected. The remaining acceptance is an actual-data personal run with Electron stopped, native Fly sync, notifications/Chrome effects, and an equal-data tray observation. Login startup, Data API enhancement and automatic native updating remain unimplemented and are shown honestly in the personal UI.

The connected isolated run has passed. `native/prepare-personal.ps1` now stages a SHA-checked copy, imports and verifies the SQLite data, creates and verifies the first backup, then moves the pair to `%LOCALAPPDATA%/LivePulseNative` only while both apps are stopped. The current installed Electron app is running, so the script correctly refused to prepare its DB; no switch occurred. Keep the original JSON and its backups for a manual return. Public installer/update compatibility remains deferred. No native authenticated Fly request has been made; fake HTTP tests alone do not prove service sync.

## Personal executable and latest verification
