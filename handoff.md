...
Still required: 15–60 minute clock-driven polling on an isolated copy with actual network and archive growth, periodic validated backups and writer coordination, real Windows notification/Chrome integration, cloud sync, full UI bridge, single-instance/update/install compatibility and equivalent-data memory comparison. No installer or version was published; production Electron v1.12.1 and its update feed are unchanged.
...
This is not the production storage path. The manual backup/recovery helper is separate from normal writes and startup. The runtime store still does not compare the source hash against the original JSON when opening, convert runtime rows for rollback, coordinate with Electron single-instance locking, query full local/cloud archives for the UI, connect polling to the tray, or deliver Windows notifications/Chrome openings. Its per-page and state-only design has no measured equivalent-data tray memory result yet. Installed Electron data and the v1.12.1 update feed remain unchanged.
