...
This is not the production storage path. The isolated runner now creates normal write checkpoints, while restore remains a manual offline operation. The runtime store still does not compare the source hash against the original JSON when opening, convert runtime rows for rollback, coordinate with Electron single-instance locking, query full local/cloud archives for the UI, or deliver Windows notifications/Chrome openings. Polling connects to the tray only by explicit isolated opt-in. Its per-page and state-only design has no measured equivalent-data tray memory result yet. Installed Electron data and the v1.12.1 update feed remain unchanged.
