...
Last maintained: 2026-09-24 after the isolated native backup checkpoint; tray-memory priority, immediate UI disposal, storage recovery and compatibility gates remain. Production remains Electron v1.12.1.
...
- `native/LivePulse.Windows` is an unshipped lifecycle prototype. Its fixture bridge and empty dashboard are not production UI. An opt-in `--hidden --isolated-monitor-db ABSOLUTE_PROBE_PATH` now runs the native public-page scheduler only on an existing `artifacts/migration-isolated-data/*.probe.sqlite` copy. Its effect sink records counts and never displays a Windows notification or opens Chrome. It has no cloud sync, updater or production data path. Keep its data/profile isolated from the installed app and do not publish it through the Electron update feed.
