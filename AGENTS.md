...
Last maintained: 2026-09-25 after building the personal native executable and verifying the isolated UI path. Production remains Electron v1.12.1.
...
- Complete connected slices: (1) background monitor/cloud sync with a shared store and backup, (2) real existing UI/analytics/settings/import bridge, (3) personal Windows execution and a concise real-data acceptance run.
...
- `native/LivePulse.Windows` remains unshipped. An opt-in `--hidden --isolated-monitor-db ABSOLUTE_PROBE_PATH` runs public monitoring and cloud polling on an ignored `*.probe.sqlite` copy with Windows effects suppressed. Its actual WebView supports channel/settings/refresh/import actions and selected histories. A separate explicit `--personal-db` mode requires an existing verified DB plus `.bak.1` under `%LOCALAPPDATA%/LivePulseNative`, refuses a running Electron app and enables notification/Chrome effects. The published personal executable passed isolated UI actions and WebView disposal, but has not switched the installed profile or made a native authenticated Fly request. Normal startup still shows a fixture. Do not publish through the Electron feed.
...
- `NativeCloudSync` reads the existing Fly HTTPS URL/token and archive cursor without exposing the token, accepts only an allowlisted Fly host, bounds requests/pages, and writes downloaded cloud observations in separate runtime tables before advancing the cursor. Its production HTTP client rejects redirects. The isolated WPF tray now calls it each minute and shares the monitor's backup checkpoint; an error appears in tray text and retries next minute. Fake HTTP tests cover page behavior; no native authenticated Fly GET has been performed. Full archive projection, real-data cloud comparison and rollback are still incomplete. Never use the installed profile or claim production cloud parity from this proof.
...
- `NativeMonitorScheduler` reads only channel IDs and validated settings each sweep, starts after 250 ms, runs channels sequentially and contains channel failures. Isolated and explicit personal DB modes start it beside one-minute cloud polling. No installed data has been connected. Short public sweeps are not a long-running equal-data memory gate.
